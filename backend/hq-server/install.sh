...
NGINX_SERVICE_FILE_PATH_NAME=/etc/nginx/conf.d/merz-pms-server-service-url.inc
...
PRODUCTION_PATH=/var/www/server/merz-pms-server
# pom.xml 과 반드시 같은 pattern 이어야 한다.
SERVER_IMAGE_NAME_PATTERN=merz-pms-server-[0-9.]*-SNAPSHOT.jar
