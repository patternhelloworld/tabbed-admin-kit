package io.github.patternhelloworld.common.config.response.error;


import io.github.patternhelloworld.common.config.database.SelectablePersistenceConst;
import io.github.patternhelloworld.common.config.response.error.dto.CommonErrorResponsePayload;
import io.github.patternhelloworld.common.config.response.error.exception.data.*;
import io.github.patternhelloworld.common.config.response.error.exception.file.FileNotFoundException;
import io.github.patternhelloworld.common.config.response.error.exception.payload.DaouHandledException;
import io.github.patternhelloworld.common.config.response.error.exception.payload.SearchFilterException;
import io.github.patternhelloworld.common.config.response.error.message.GeneralExceptionMessage;
import io.github.patternhelloworld.common.util.OrderConstants;
...
public class PmsCommonGlobalExceptionHandler {
    ...
    public ResponseEntity<?> resourceNotFoundException(ResourceNotFoundException ex, WebRequest request) {
        ...
        CommonErrorResponsePayload commonErrorResponsePayload;
        if(ex.getErrorMessages() != null){

            commonErrorResponsePayload = new CommonErrorResponsePayload(ex.getErrorMessages(),
                    ex, request.getDescription(false), CustomExceptionUtils.getAllStackTraces(ex),
                    CustomExceptionUtils.getAllCauses(ex), null);

        }else{
            commonErrorResponsePayload = new CommonErrorResponsePayload(ex.getMessage(), request.getDescription(false),
                    ex.getMessage(), CustomExceptionUtils.getAllStackTraces(ex),
                    CustomExceptionUtils.getAllCauses(ex));
        }
        return new ResponseEntity<>(commonErrorResponsePayload, HttpStatus.NOT_FOUND);
    ...
    public ResponseEntity<?> noUpdateTargetException(NoUpdateTargetException ex, WebRequest request) {
        CommonErrorResponsePayload commonErrorResponsePayload = new CommonErrorResponsePayload(ex.getMessage(), request.getDescription(false),
    ...
    public ResponseEntity<?> searchFilterException(SearchFilterException ex, WebRequest request) {
        ...
        CommonErrorResponsePayload commonErrorResponsePayload = new CommonErrorResponsePayload(ex.getCause().getMessage(), request.getDescription(false),
                ex.getMessage(), ex.getStackTrace()[0].toString());
        return new ResponseEntity<>(commonErrorResponsePayload, HttpStatus.BAD_REQUEST);
    }
    ...
    public ResponseEntity<?> preconditionFailedException(PreconditionFailedException ex, WebRequest request) {
        CommonErrorResponsePayload commonErrorResponsePayload = new CommonErrorResponsePayload(ex.getMessage(), request.getDescription(false),
                ex.getMessage(), CustomExceptionUtils.getAllStackTraces(ex),
                CustomExceptionUtils.getAllCauses(ex));
        return new ResponseEntity<>(commonErrorResponsePayload, HttpStatus.CONFLICT);
    }
    ...
    public ResponseEntity<?> methodArgumentNotValidException(MethodArgumentNotValidException ex, WebRequest request) {
        ...
        return new ResponseEntity<>(commonErrorResponsePayload, HttpStatus.UNPROCESSABLE_ENTITY);
    }
    ...
    public ResponseEntity<?> bindExceptionHandler(BindException ex, WebRequest request) {
        ...
        CommonErrorResponsePayload commonErrorResponsePayload = new CommonErrorResponsePayload(ex.getMessage(), request.getDescription(false), null,
                errorMessages,
                CustomExceptionUtils.getAllStackTraces(ex), CustomExceptionUtils.getAllCauses(ex));
        return new ResponseEntity<>(commonErrorResponsePayload, HttpStatus.UNPROCESSABLE_ENTITY);
    }
    ...
    * */
    ...
    public ResponseEntity<?> dataIntegrityViolationException(DataIntegrityViolationException ex, WebRequest request) {
        ...
        if(ex.getMostSpecificCause().getMessage() != null
            ...
            /*  UNIQUE, NULL */
            if(dbDialect.equals(SelectablePersistenceConst.MYSQL_8.getValue()) || dbDialect.equals("io.github.patternhelloworld.tak.config.database.dialect.CustomMySQL8Dialect")) {
                // 1. UNIQUE 부터 검사
    ...
    public ResponseEntity<?> restTemplateAccessException(ResourceAccessException ex, WebRequest request) {
        ...
        return new ResponseEntity<>(commonErrorResponsePayload, HttpStatus.REQUEST_TIMEOUT);
    }
    ...
    public ResponseEntity<?> unhandledExceptionHandler(Exception ex, WebRequest request) {
        ...
        if (allStackTraces.contains("SocialOauthWeb.java")) {
            commonErrorResponsePayload = new CommonErrorResponsePayload("", request.getDescription(false), GeneralExceptionMessage.UNHANDLED_ERROR.getUserMessage(),
                    "", "");
        }else{
            commonErrorResponsePayload = new CommonErrorResponsePayload(ex.getMessage(), request.getDescription(false), GeneralExceptionMessage.UNHANDLED_ERROR.getUserMessage(),
                    CustomExceptionUtils.getAllStackTraces(ex), CustomExceptionUtils.getAllCauses(ex));
        }
        return new ResponseEntity<>(commonErrorResponsePayload, HttpStatus.INTERNAL_SERVER_ERROR);
    }
