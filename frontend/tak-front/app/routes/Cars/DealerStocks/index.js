...
import CarsDealerStocksUpdate from "./One/DealerStocksUpdate";

import LoadingOverlay from 'react-loading-overlay'
import ClockLoader from 'react-spinners/ClockLoader';
import {Link} from "react-router-dom";
import {CRUD_COLUMNS, isAuthorized} from "../../../shared/utils/authorization";
import CarsDealerStocksCreate from "./One/DealerStocksCreate";
...
import getDealerStocksColumns from "./columns";
import {

    fetchSecondDepthDepts,
...
const PK_NAME = "dealerStockIdx";

const CarsDealerStocks = ({recoilKey, ...props}) => {
    ...
    * */
    ...
    const columns = React.useMemo(() => getDealerStocksColumns({ PK_NAME}), []);
    ...
    * */
    ...
    const {data, error, isError, isFetching, isLoading} = useMantineFetch({
        ...
        pagination,
        sorting,
        fetchFunc: agent.DealerStock.fetch,
        cacheKey: recoilKey,
        additionalSearchFilter: {...createSearchFilter(), deptIdx : selectedSecondDepthDeptIdx,
                        ...
                        <Col className={"ml-4"}>
                            ...
                            >
                                <Input
                                    ...
                                    defaultValue={""}
                                    onChange={(e) => setSelectedSecondDepthDeptIdx(e.target.value)}
                                    required
                                >
    ...
    * */
    ...
    const onePrevCycle = async ({localOne}) => {
        ...
        try {
            setOne({
                ...localOne, meta: {
                    secondDepthDepts : [...secondDepthDepts]
                }
            });
            ...
            >
                <Row>
                    <Col xl={12} >
                        ...
                        >
                            {isCreateOne(one, PK_NAME) ? <CarsDealerStocksCreate refreshAll={refreshAll} refreshOne={refreshOne}
                                                     recoilKey={recoilKey} PK_NAME={PK_NAME}/> : ""}
                            {isUpdateOne(one, PK_NAME) ? <CarsDealerStocksUpdate createOne={createOne} refreshAll={refreshAll}
...
export default CarsDealerStocks;