...
const ProjectIssueSearch = ({ project }) => {
  ...
  const [{ data, isLoading }, fetchIssues] = useApi.get('/api/v1/issues', {}, { lazy: true });
...
const renderIssue = issue => (
  <Link key={issue.id} to={`/project/board/issues/${issue.id}`}>
    <Issue>
      <IssueTypeIcon issueTypeId={issue.issueTypeId} size={25} />
      <IssueData>
        <IssueTitle>{issue.title}</IssueTitle>
        <IssueTypeId>{`${issue.issueTypeId}-${issue.id}`}</IssueTypeId>
      </IssueData>
    </Issue>
  </Link>
);
