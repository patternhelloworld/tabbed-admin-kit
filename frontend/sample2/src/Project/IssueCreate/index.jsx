...
const ProjectIssueCreate = ({ project, fetchProject, onCreate, modalClose  }) => {
  const [{ isCreating }, createIssue] = useApi.post('/api/v1/issues');
  ...
  return (
    ...
    >
      <FormElement>
        <FormHeading>이슈 생성하기</FormHeading>
        <Form.Field.Select
          name="issueTypeId"
          label="Issue Type"
...
const renderType = ({ value: issueTypeId }) => (
  <SelectItem>
    <IssueTypeIcon issueTypeId={issueTypeId} top={1} />
    <SelectItemLabel>{IssueTypeCopy[issueTypeId]}</SelectItemLabel>
  </SelectItem>
);
